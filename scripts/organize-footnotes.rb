...
def remove_quotations_with_following_parentheses(text)
  ...
  debug = false
  while i < text.length
    puts "At position #{i}, char is '#{text[i]}', quotation_count is #{quotation_count}" if debug
    if text[i] == '"' && (i == 0 || text[i-1] =~ non_text)
      puts "Found opening quotation at #{i}" if debug
      quotation_count = 1
      j = i + 1
      while j < text.length
        if text[j] == '"' && text[j+1] =~ non_text
      ...
      puts text[i..j-1] if debug
      puts "Matching text: '#{text[j..-1]}' for parentheses" if debug
      match = text[j..-1].match(/^\s*\(([^)]+)\)/)
      if match
        ...
        puts "Removed text: '#{text[i..(j + match[0].length - 1)]}'" if debug
        i = j + match[0].length

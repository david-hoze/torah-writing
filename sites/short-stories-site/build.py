...
# -*- coding: utf-8 -*-
"""
Builds a static site for the "short-stories" collection from Markdown files.

Usage:
    python3 build.py [SOURCE_DIR] [OUTPUT_DIR]

SOURCE_DIR  folder containing one .md file per story (default: ../short-stories)
OUTPUT_DIR  where the site is written (default: ./site)

Each story's first "# " heading becomes its title. An optional
...
adding or editing a story, just run this script again.
"""
import os, re, sys, shutil
import markdown

# ---- site branding (edit freely) ----
SITE_TITLE    = "סיפורים קצרים"
SITE_SUBTITLE = "סיפורת"
# -------------------------------------

# ---- sister sites (cross-links shown in the footer) ----
SITE_KEY = "short-stories"
SITES = [
...
SRC  = os.path.abspath(sys.argv[1]) if len(sys.argv) > 1 else os.path.join(HERE, "..", "..", "short-stories")
...
def collect_stories(src):
    """Return [(slug, path)] for every story .md, sorted by slug."""
    out = []
...
def parse_file(path):
    ...
    # stories are written in Obsidian (so e.g. dialogue lines stay separate).
    ...
    # own content: Hebrew stays RTL, while English blocks render LTR/left-aligned
...
def render_index(stories):
    rows = []
    for slug, title, subtitle, _ in stories:
...
<nav class="toc" aria-label="רשימת סיפורים">
  <ol class="toc-list">
    {"".join(rows)}
  </ol>
</nav>"""
...
<nav class="chap-nav">
  <a class="np np-prev" href="/"><span class="np-k">חזרה</span><span class="np-t">כל הסיפורים</span></a>
...
def main():
    if not os.path.isdir(SRC):
        sys.exit(f"Source folder not found: {SRC}\n"
                 ...
                 f"  python3 build.py /path/to/short-stories")
    raw = collect_stories(SRC)
    if not raw:
        sys.exit(f"No story .md files found in {SRC}")

    stories = []
    for slug, path in raw:
        title, subtitle, html = parse_file(path)
        stories.append((slug, title, subtitle, html))
    ...
    with open(os.path.join(OUT, "index.html"), "w", encoding="utf-8") as f:
        f.write(render_index(stories))

    for slug, title, subtitle, html in stories:
        d = os.path.join(OUT, slug)
        os.makedirs(d, exist_ok=True)
        with open(os.path.join(d, "index.html"), "w", encoding="utf-8") as f:
            f.write(render_story(title, subtitle, html))

    css_src = os.path.join(HERE, "style.css")
    if os.path.exists(css_src):
        shutil.copy(css_src, os.path.join(OUT, "style.css"))

    print(f"Built {len(stories)} stories + index -> {OUT}")
    print("Stories:", ", ".join(s[0] for s in stories))
